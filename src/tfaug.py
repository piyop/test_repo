...
class DatasetCreator():
    ...
    def from_path(self, img_paths: List[str],
        ...
        # set label format
        label_type, num_feature = get_feature_type(labels)
    ...
    def _get_inputs_shapes(self, ds, label_type, num_inputs):
        ...
        if (len(inputs) > 1 and
            ...
            for data_in in inputs[:num_inputs]:
    ...
    def from_tfrecords(self,
        ...
        ds_aug = self._get_aug_ds(ds_aug, label_type, len(imgs_dtype),
                                  data_names=example_formats.keys())

        return ds_aug.prefetch(AUTOTUNE), num_img

    def _get_aug_ds(self, ds_org, label_type, num_inputs, data_names=None):

        inputs_shape, input_labels_shape = self._get_inputs_shapes(
            ds_org, label_type, num_inputs)
...
class TfrecordConverter():
    ...
    def from_path_label(self, path_imgs: List[str],
        ...
        if label_type == 'segmentation':
            ...
            reader_label = lambda x : [np.array(x)]
    ...
    def from_ary_label(self, ary: Union[List[np.ndarray], np.ndarray],
        ...
        if label_type == 'segmentation':
            if n_lblin > 1:
                def reader_label(x): return [np.array(i) for i in x]
            else:
                def reader_label(x): return [np.array(x)]
        elif label_type == 'class':
            def reader_label(x): return [np.array(x)]
    ...
    def _exformat_from_array(self, iimg, label_type, ilabel=None):
        ...
        if ilabel is not None:
            ...
            for i, label in enumerate(ilabel):
                ...
                if ilabel is not None:
                    feature[f'label_in{i}'] = label_feature(label)
                    if label_type == 'segmentation':
